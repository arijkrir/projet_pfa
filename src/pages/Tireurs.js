...
import { Button, Modal, Paper, Table, TableBody, TableCell, InputLabel, TableContainer, TableHead, TableRow, TextField, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Select, MenuItem, FormControl } from '@mui/material';
...
const TireursPage = () => {
  const [tireurs, setTireurs] = useState([
    { id: 1, nom: 'ali', prenom: 'saidi', grade: 'Sergent', nombreTirs: 50 },
    { id: 2, nom: 'mohsen', prenom: 'hsan', grade: 'Capitaine', nombreTirs: 75 },
    { id: 3, nom: 'ahmed', prenom: 'gharbi', grade: 'Lieutenant', nombreTirs: 40 },
  ]);
  ...
  const confirmDeleteTireur = () => {
    if (deleteId !== null) {
      const nouveauListeTireurs = tireurs.filter((tireur) => tireur.id !== deleteId);
  ...
  const ajouterTireur = () => {
    if (nouveauNom && nouveauPrenom && nouveauGrade) {
      ...
      setTireurs([...tireurs, nouveauTireur]);
      toggleModal(); 
      setNouveauNom('');
      setNouveauPrenom('');
      setNouveauGrade('');
      ...
      <div style={{ padding: '10px', marginTop: '10px', textAlign: 'center', marginLeft: '250px' }}>
        ...
        <Dialog open={editDialogOpen} onClose={closeEditDialog}>
          ...
          <DialogActions>
            ...
            <Button onClick={confirmEditTireur} style={{ color: 'green' }}>Enregistrer</Button>
          </DialogActions>
        </Dialog>
  ...
  <IconButton onClick={() => openDeleteDialog(tireur.id)}>
